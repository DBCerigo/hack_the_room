...
window.BFtObject= function () {
        ...
        object.setupSocket = object.start = function(socketAddress){
			...
			if (BrowserDetect.browser == "Firefox") {
				...
				object.socket = new ReconnectingWebSocket(get_appropriate_ws_url(socketAddress), null, {debug: false, reconnectInterval: 5000,maxReconnectInterval:5100,timeoutInterval:4000,maxReconnectAttempts:10});
				object.retrial=0;
			}
			...
			try {
				...
				object.socket.onmessage =function got_packet(msg) {
					...
					else if(messageObject["m"] == "magnetometerUpdate")
					{
						...
						try{
					        magnetometerEvent(object.magnetometer,object.magnetometerIntensity);
						}
						catch(e)
						{
					...
					else if(messageObject["m"] == "distanceChanged")
					{
						...
						try{
					        distanceEvent(object.proximity);
						}
						catch(e)
						{
					...
					else if(messageObject["m"] == "audioJackChanged")
					{
						...
						try{
					        distanceEvent(object.audiojack);
						}
						catch(e)
						{
					...
					else if(messageObject["m"] == "powerSourceChanged")
					{
						...
						try{
					        distanceEvent(object.powersource);
						}
						catch(e)
						{
	    ...
	    object.registerDistance = function()
	    {
		...
		object.registerAttitude = function(frequency)
		{
		...
		object.registerAudioJack=function ()
	    {
	    ...
	    object.registerMagnetometer=function ()
	    {
		...
		object.registerOrientation = function()
		{
		...
		object.registerRxEmbedded = function(){
			var message="{\"m\":\"srx\"}";
			this.sendMessage(message);
		}
